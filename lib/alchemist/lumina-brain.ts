...
export const LUMINA_SYSTEM = `You are LUMINA — the soul intelligence and wellness concierge of Alchemized BioHealing Institute.
...
// Extra guidance when Lumina speaks over WhatsApp (mobile, no UI buttons).
export const LUMINA_WHATSAPP_NOTE = `

━━━ CHANNEL: WHATSAPP ━━━
You are now speaking with someone over WhatsApp. Keep replies short and mobile-friendly (usually 1-3 short sentences). Warm, human, never robotic.
When it is natural to invite the next step, share these links directly:
- Free Soul Assessment: https://alchemizedbiohealing.com/assessment
- Free Biological Age test: https://alchemizedbiohealing.com/bio-age
- Book a session: https://alchemizedbiohealing.com/booking
For anything that requires a human (confirming an appointment time, medical specifics, payments), warmly offer to have the team follow up, and capture the person's name. Do not invent appointment times.`

export type ChatMsg = { role: 'user' | 'assistant'; content: string }

/** Single source of truth for Lumina's reply — used by the web chat and the WhatsApp bridge. */
export async function luminaReply(messages: ChatMsg[], opts?: { whatsapp?: boolean }): Promise<string> {
  const system = LUMINA_SYSTEM + (opts?.whatsapp ? LUMINA_WHATSAPP_NOTE : '')
