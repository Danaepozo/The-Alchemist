...
export default function LyraBasePage() {
  ...
  async function send() {
    ...
    try {
      const res = await fetch('/api/lyra-base', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ messages: next, code: accessCode }) })
  ...
  if (!unlocked) {
    return (
      <div style={{ minHeight: '100vh', background: 'radial-gradient(ellipse at 50% -10%, #1a1535 0%, #0a0a14 45%, #050509 100%)', color: CREAM, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem 1.25rem' }}>
        ...
        <p style={{ fontSize: '0.72rem', letterSpacing: '0.3em', color: 'rgba(240,232,216,0.45)', textTransform: 'uppercase', margin: '0.6rem 0 2rem' }}>Base · Espacio por invitación</p>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'radial-gradient(ellipse at 50% -10%, #1a1535 0%, #0a0a14 45%, #050509 100%)', color: CREAM, display: 'flex', flexDirection: 'column' }}>
      <header style={{ textAlign: 'center', padding: '2.5rem 1.5rem 1.5rem', borderBottom: '1px solid rgba(201,150,60,0.15)' }}>
        ...
        <div style={{ fontSize: '0.7rem', letterSpacing: '0.3em', color: 'rgba(240,232,216,0.45)', textTransform: 'uppercase', marginTop: '0.5rem' }}>Versión Base · Holistic Wisdom · Bella&rsquo;s Space</div>
      </header>
